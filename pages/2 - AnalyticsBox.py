...
bn1, b52, b25, bn2, bn3, bn4 = st.columns([4, 2, 2, 2, 2, 3])
with bn1:
...
with st.container():
    cvx1, cvx2, cvx3, cvx4, cvx5 = st.columns([3, 2, 2, 2, 2])
    with cvx1:
    ...
    st.write("Compare the spread between any asset's high and low price limits.")
with st.container():
...
with st.container():
    l_dr, fig_dr, l_dlr, delta_r, fig_dlr = dr(df)
    l_cr, fig_cr = cr(df)
    cr1, cr2, cr3, cr4, cr5 = st.columns([3, 2, 2, 2, 2])
    with cr1:
...
st.write("  --------------  ")
with st.empty():
    cvv1, cvv2, cvv3, cvv4, cvv5 = st.columns([3, 2, 2, 2, 2])
    with cvv1:
...
st.write("Infer market's conviction. Gauge accumulation tendencies, upcoming reversals.")
with st.container():
        ...
        try:
            ...
            df_risk = pd.DataFrame({'Risk Types': ["Audit Risk", "Board Risk",
                                    ...
                                    'Risk Levels': [auditRisk, boardRisk,
                                                    ...
                                                    compensationRisk]})
            df_risk = df_risk.set_index(df_risk['Risk Types'])
            figure_risk = px.bar(df_risk,
                df_risk["Risk Levels"], labels=df_risk['Risk Types'], colors=df_risk['Risk Types'])
#            figure_risk.update_layout(height=300, title='Institutional Risk Map',  title_x=0.25,
#                showlegend=False)
#
