...
with st.container():
    ...
    with j1:
        st.write(' ')
    with j2:
        st.write("Value Proposition & Service Offering")
    ...
    with j3:
        st.write(' ')
    k1, k2, k3 = st.columns([1, 6, 1])
    with k1:
        st.write(' ')
    with k2:
        st.write("Business Model Canvas")
        vfile2 = open(f"{direc}/pages/appdata/imgs/F2B_Ledgr_s_BMC.mp4", "rb")
        vbytes2 = vfile2.read()        
        st.video(vbytes2, format="mp4", start_time=0,
                 loop=True, autoplay=False, muted=False)
    with k3:
        st.write(' ')
st.write("-------------------------------------------------------------------")
