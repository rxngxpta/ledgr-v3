...
stock_price = ticker.history(period='2y')['Close'][1]
# stock_price = stock_price.last()
st.write("Stock Price", round(stock_price, 3))
...
with h15:
    st.metric("Interest Coverage Ratio", round(icr, 4))
st.write('-------------------------------------------------------------------')
try:
...
with g15:
    st.metric("Debt-Service-Coverage-Ratio", round(dscr, 4))
st.write('-------------------------------------------------------------------')
#################################################
...
with g43:
    st.metric("""=>BV in Cr.=Total Stockholders' Equity/(Total Outstanding Shares)""",
             book_value/10000000)
